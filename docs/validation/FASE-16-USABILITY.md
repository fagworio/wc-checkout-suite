...
| Prova | Resultado |
|---|---|
| Observador FASE-16 legado (aposentado) | **16/0** no cenário original: as quatro telas abriam com um título que dizia o que decidiam e com a ação principal visível (e as Configurações, sem ação, afirmada como tal); a lista de campos era agrupada pela secção que os preenchia e o checkout da loja era oferecido por cima dela; o ecrã de status respondia que um estado não cobra nada e nomeava o interruptor «Estado antes do pagamento»; o ecrã de automação respondia que uma automação não cobra nada e o passo do pagamento só oferecia o que a loja executava; e nenhuma tela mostrava o vocabulário do código (`container`, `FieldBinding`, `FieldDefinition`). Resultado preservado como histórico; o script foi substituído pela prova UX-008. |
