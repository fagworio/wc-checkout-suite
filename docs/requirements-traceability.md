...
>
> **F14 (WCCS-071…076)** fechou a revisão de vínculos e exibição por destino: coleta, vinculação e exibição são três decisões separadas, os destinos começam desativados e nenhuma área recebe painel sem configuração explícita. A auditoria por área (`docs/validation/WCCS-076.md`) prova as duas direções — o que não foi vinculado não aparece em lugar nenhum, e o que foi aparece na seção, com o título e na ordem que o vínculo configura —, e os dois destinos que a mesma página servia (detalhe do pedido e página de agradecimento) passaram a ser decididos pela página. O destino `customer_profile` deixou de ser inerte e foi depois dividido em `customer_account` (página do próprio cliente em Minha Conta) e `admin_customer` (painel no perfil do utilizador, para a equipa), com o mesmo armazenamento do cliente e migração assistida (`docs/validation/DESTINOS-MINHA-CONTA-E-PERFIL-ADMIN.md`). Uma seção oferecida numa dessas áreas passa a ser uma página de Minha Conta — endpoint, entrada no menu, formulário validado pelo mesmo `ValueProcessor` do checkout e valor guardado no cliente —, e o documento é recusado (`account_section_requires_customer_storage`) quando o campo vinculado a essa página não guarda no cliente. A prova está em `tests/Integration/ACCOUNT-my-account-sections-proof.php` (30/0) e no percurso de browser `tests/browser/my-account-sections-flow.mjs` (6/6); o que ainda falta na conta (upload sem pedido, reuso de uma página existente) está na especificação `roadmap/ESPECIFICACAO-SECOES-WCCS.md`.
