...
await prototype.setContent( readFileSync( 'roadmap/fields.html', 'utf8' ), {
	waitUntil: 'load',
} );
await prototype.waitForTimeout( 600 );
await prototype.screenshot( {
	path: `${ OUT }/prototype.png`,
	fullPage: false,
} );
...
for ( const pair of [ COOKIE, process.env.WCCS_AUTH_COOKIE || '' ].filter(
	Boolean
) ) {
...
page.on( 'pageerror', ( error ) =>
	errors.push( String( error.message ).slice( 0, 160 ) )
);
...
const said = await page.evaluate( () => ( {
	...
	navItems: Array.from(
		document.querySelectorAll( '.wccs-admin .nav button span' )
	).map( ( element ) => element.textContent ),
	actions: Array.from(
		document.querySelectorAll( '.wccs-admin .top-actions button' )
	).map( ( element ) =>
...
const adminPreview = await preview.evaluate( () => ( {
	...
	caption:
		document.querySelector( '#previewView .preview-caption' )
			?.textContent ?? '',
	brand:
		document.querySelector( '#previewView .store-brand' )?.textContent ??
		'',
	device:
		document.querySelector( '#previewView .preview-shell' )?.className ??
		'',
} ) );
