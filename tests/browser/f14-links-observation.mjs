...
async function openField( listSection, fieldId ) {
	await page.getByRole( 'button', { name: new RegExp( `^${ listSection }` ) } ).first().click();
	await page.waitForTimeout( 500 );

	await page.locator( `text=${ fieldId }` ).first().click();
	await page.waitForTimeout( 800 );

	await page.getByRole( 'button', { name: 'Vínculos', exact: true } ).first().click();
...
async function panel() {
	return page.evaluate( () => {
		const inspector = document.querySelector( '.inspector' ) || document.body;

		return {
			destinations: Array.from( inspector.querySelectorAll( '[role=group][aria-label]' ) )
				.map( ( node ) => node.getAttribute( 'aria-label' ) )
				.filter( ( label ) => label && label !== 'Propriedades do campo' ),
			checkboxes: Array.from( inspector.querySelectorAll( 'input[type=checkbox]' ) ).map( ( node ) => ( {
				id: node.id,
				name: node.getAttribute( 'aria-label' ) || node.id,
				checked: node.checked,
			} ) ),
			selects: Array.from( inspector.querySelectorAll( 'select' ) ).map( ( node ) => ( {
				id: node.id,
				value: node.value,
				options: Array.from( node.options ).map( ( option ) => option.value ),
			} ) ),
			inputs: Array.from( inspector.querySelectorAll( 'input[type=text], input[type=number]' ) ).map(
...
const value = ( list, id ) => ( list.find( ( entry ) => entry.id === id ) || {} ).value;
const held = ( list, id ) => ( list.find( ( entry ) => entry.id === id ) || {} ).checked;
const actions = ( snapshot, destination ) =>
	snapshot.checkboxes
		.filter( ( box ) => box.id.startsWith( `wccs-link-action-${ destination }-` ) )
		.map( ( box ) => box.id.replace( `wccs-link-action-${ destination }-`, '' ) )
		.sort();

await page.goto( URL, { waitUntil: 'domcontentloaded' } );
await page.waitForTimeout( 2500 );

record( 'The screen the merchant opens is served', ( await page.title() ) !== '', await page.title() );
...
await step( 'The links tab of a file field could be opened', async () => {
	...
	record(
		...
		file.destinations.length === 7,
		file.destinations.join( ' | ' )
	);

	record(
		'A destination that is off offers only its switch',
		! file.selects.some( ( select ) => select.id === 'wccs-link-section-customer_profile' ) &&
			! file.checkboxes.some( ( box ) => box.id.startsWith( 'wccs-link-action-customer_profile-' ) ),
		'customer_profile and public_api are off'
	);

	record(
		'The actions offered are the ones that destination may perform',
		JSON.stringify( actions( file, 'admin_order' ) ) ===
			JSON.stringify( [ 'approve', 'download', 'resubmit', 'show_metadata', 'view' ] ) &&
			JSON.stringify( actions( file, 'customer_order' ) ) ===
				JSON.stringify( [ 'download', 'resubmit', 'show_metadata', 'view' ] ) &&
			JSON.stringify( actions( file, 'customer_email' ) ) ===
				JSON.stringify( [ 'download', 'show_metadata', 'view' ] ),
		'admin=' + actions( file, 'admin_order' ).join( ',' ) + ' customer=' + actions( file, 'customer_order' ).join( ',' )
	);

	record(
		'Reviewing is offered to staff and not to the customer',
		actions( file, 'admin_order' ).includes( 'approve' ) && ! actions( file, 'customer_order' ).includes( 'approve' ),
	...
	record(
		...
		value( file.selects, 'wccs-link-section-admin_order' ) === 'documentos_para_analise' &&
			value( file.inputs, 'wccs-link-title-admin_order' ) === 'Autorização assinada' &&
			value( file.inputs, 'wccs-link-position-admin_order' ) === '20' &&
			value( file.selects, 'wccs-link-section-order_received' ) === 'documentos_enviados' &&
			value( file.inputs, 'wccs-link-title-order_received' ) === 'Enviado agora' &&
	...
	record(
		...
		held( file.checkboxes, 'wccs-link-action-admin_order-approve' ) &&
			held( file.checkboxes, 'wccs-link-action-customer_order-download' ) === false &&
			held( file.checkboxes, 'wccs-link-action-admin_email-download' ),
		'the customer may look, not take; the store may take'
	);

	const staffSection = file.selects.find( ( select ) => select.id === 'wccs-link-section-admin_order' ) || {};
...
await step( 'The links tab of a text field could be opened', async () => {
	...
	record(
		...
		text.destinations.length === 7 &&
			! text.saysActions &&
			! text.checkboxes.some( ( box ) => box.id.startsWith( 'wccs-link-action-' ) ),
		'link switches=' +
			text.checkboxes.filter( ( box ) => box.id.startsWith( 'wccs-link-' ) ).length +
			' actions heading=' +
			text.saysActions
	);
	...
	record(
		...
		! text.checkboxes.some(
			( box ) => box.id.startsWith( 'wccs-link-' ) && ! box.id.includes( 'action' ) && box.checked
		),
...
// 3. The approval flow.
// ---------------------------------------------------------------------------
await step( 'The approval flow of the reviewed field could be read', async () => {
	...
	record(
		...
		held( reviewed.checkboxes, 'wccs-approval-required' ) &&
			value( reviewed.inputs, 'wccs-approval-status' ) === 'Pendente de aprovação' &&
			value( reviewed.selects, 'wccs-approval-area' ) === 'admin_order' &&
			value( reviewed.inputs, 'wccs-approval-section' ) === 'documentos_para_analise',
		'area=' +
	...
	record(
		...
		reviewed.destinations.length === 7 && reviewed.saysActions === false,
		'the fiscal document is a text field: a link, and no file actions'
	);
} );

await step( 'Asking for a review on a field that has none offered a state', async () => {
	await openField( 'Cobrança', 'campo_sem_vinculo' );

	await page.getByRole( 'checkbox', { name: 'Exigir análise manual' } ).first().check();
	...
	record(
		...
		value( asked.inputs, 'wccs-approval-status' ) === 'Pendente de aprovação' &&
...
await step( 'The section dialog could be opened', async () => {
	...
	const dialogAreas = await page.evaluate( () =>
		Array.from( document.querySelectorAll( 'input[type=checkbox][id^=wccs-new-section-area-]' ) ).map(
			( node ) => ( { id: node.id, checked: node.checked } )
		)
	);
	...
	record(
		...
		dialogAreas.length === 7 &&
			dialogAreas.some( ( area ) => area.id === 'wccs-new-section-area-checkout' && area.checked ) &&
			dialogAreas.some( ( area ) => area.id === 'wccs-new-section-area-admin_order' && ! area.checked ) &&
			! dialogAreas.some( ( area ) => area.id === 'wccs-new-section-area-public_api' ),
		'areas=' + dialogAreas.map( ( area ) => area.id.replace( 'wccs-new-section-area-', '' ) ).join( ',' )
	);
...
await step( 'The draft could be published from the interface', async () => {
	await page.getByRole( 'button', { name: 'Revisar publicação' } ).first().click();
	await page.waitForTimeout( 900 );

	const review = page.locator( 'dialog[open]' ).first();
	const reviewText = ( await review.count() ) > 0 ? await review.innerText() : '';
	...
	const publishButton = page.getByRole( 'button', { name: /Publicar alterações|Publish changes/ } ).first();

	record( 'The interface offers the publication', ( await publishButton.count() ) > 0, 'the button exists' );
