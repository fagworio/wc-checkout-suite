...
const adminUser = process.env.WCCS_ADMIN_USER || '';
const adminPassword = process.env.WCCS_ADMIN_PASSWORD || '';
const results = [];
...
try {
	await page.goto(
		`${ origin }/wp-admin/admin.php?page=wccs-checkoutsuite&section=fields&area=checkout`,
		{ waitUntil: 'networkidle', timeout: 30000 }
	);
	await page
		.locator( 'button' )
		.filter( { hasText: 'Salvar alterações' } )
		.first()
		.waitFor();
	...
	await page
		.locator( 'button' )
		.filter( { hasText: 'Nova seção' } )
		.first()
		.click();
	await page.locator( '#wccs-new-section-title' ).fill( name );
	await page
		.getByRole( 'button', { name: /Add section|Adicionar seção/ } )
		.click();
	const sectionTab = page
	...
	await page
		...
		.click();
	await page
		.locator( '#wccs-condition-root-source' )
		.selectOption( 'country' );
	await page
		.locator( '#wccs-condition-root-operator' )
		.selectOption( 'equals' );
	await page.locator( '#wccs-condition-root-value' ).fill( 'BR' );
	await page
		.locator( '.condition-result' )
		.filter( { hasText: 'BR' } )
		.waitFor();
	...
	await page
		.getByRole( 'button', { name: 'Always show this field', exact: true } )
		.click();
	await page
		.getByText( 'This field is always shown.', { exact: true } )
		.waitFor();
	...
	const renamedSectionTab = page
		.locator( '.section-tabs button' )
		.filter( { hasText: `${ name } renomeada` } );
	...
	record(
		'abre Regras do editor',
		( await page.locator( 'body' ).innerText() ).includes( 'Regras' )
	);
...
console.log(
	JSON.stringify(
		{ results, failures: results.filter( ( result ) => ! result.ok ) },
		null,
		2
	)
);
